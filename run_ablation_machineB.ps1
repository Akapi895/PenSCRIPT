...
# -- Pre-check R18 --
if (-not $SkipR18) {
    $checkResult = python -c "from run_strategy_c import parse_args; import sys; sys.argv=['x','--sim-scenarios','a','--pengym-scenarios','b','--no-canonicalization']; a=parse_args(); print('OK')" 2>&1 | Out-String
    ...
    Write-Host " Pre-check OK: --no-canonicalization flag found" -ForegroundColor Green
}
