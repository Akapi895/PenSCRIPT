...
from src.agent.util import Configure
...
class Encoder:
    ...
    def __init__(
        ...
        if SBERT_model_name:
            ...
            local_path = pretrained_model_path / SBERT_model_name
            if local_path.exists():
                model_source = str(local_path)
            else:
                # Use HuggingFace model name directly (auto-download)
                model_source = SBERT_model_name
                print(f"[INFO] Downloading SBERT model '{SBERT_model_name}' from HuggingFace...")
            
            self.SBERT_model = SentenceTransformer(model_source)
...
# Use path relative to this file instead of CWD
_encoder_dir = Path(__file__).parent
pretrained_model_path = _encoder_dir / Configure.get("Embedding", "embedding_models")
if not pretrained_model_path.exists():
    # Create folder if missing - models will be downloaded from HuggingFace
    pretrained_model_path.mkdir(parents=True, exist_ok=True)
    print(f"[INFO] Created Embedding models folder: {pretrained_model_path}")
try:
